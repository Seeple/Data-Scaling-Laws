...
# Example:
#   GPU_LIST="0,1,2,3" NUM_PROCESSES=4 bash pick_plate.sh
GPU_LIST="${GPU_LIST:-}"
...
#   MODEL_PRETRAINED=false bash pick_plate.sh
...
accelerate launch --main_process_port 29502 --config_file "${ACCELERATE_CONFIG_FILE}" "${ACCELERATE_ARGS[@]}" --mixed_precision 'bf16' ../train.py \
  ...
  task.dataset_path=../data/dataset/${task_name}/teleop_data/pick_plate_raw_4_24.zarr.zip \
