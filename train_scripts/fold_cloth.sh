...
# Example:
#   GPU_LIST="0,1,2,3" NUM_PROCESSES=4 bash fold_cloth.sh
GPU_LIST="${GPU_LIST:-}"
...
#   MODEL_PRETRAINED=false bash fold_cloth.sh
...
accelerate launch --main_process_port 29505 --config_file "${ACCELERATE_CONFIG_FILE}" "${ACCELERATE_ARGS[@]}" --mixed_precision 'bf16' ../train.py \
  --config-name=train_diffusion_unet_timm_umi_workspace \
  multi_run.run_dir=${run_dir} multi_run.wandb_name_base=${logging_time} hydra.run.dir=${run_dir} hydra.sweep.dir=${run_dir} \
  task.dataset_path=../data/dataset/fold_cloth/teleop_data/fold_cloth_raw_4_28.zarr.zip \
